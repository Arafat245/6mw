...
- Clinic: Ridge(α=5) — best single model for 55 curated features
- Home: Vote(Ridge+Lasso+SVR) — best ensemble for home features
...
def loo_ridge(X, y, alpha):
    """Clinic model: Ridge LOO, no feature selection."""
    pr = np.zeros(len(y))
...
def loo_vote_no_selection(X, y, alpha):
    """Home model: Vote(Ridge+Lasso+SVR) LOO, no feature selection."""
    pr_r = np.zeros(len(y))
    pr_l = np.zeros(len(y))
    pr_s = np.zeros(len(y))
    for tr, te in LeaveOneOut().split(X):
        ...
        pr_r[te] = Ridge(alpha=alpha).fit(X_tr, y[tr]).predict(X_te)
        pr_l[te] = Lasso(alpha=max(1, alpha // 4), max_iter=5000).fit(X_tr, y[tr]).predict(X_te)
        pr_s[te] = SVR(kernel='rbf', C=500, gamma=0.05).fit(X_tr, y[tr]).predict(X_te)
    pr = (pr_r + pr_l + pr_s) / 3
    return r2_score(y, pr), mean_absolute_error(y * FT2M, pr * FT2M), spearmanr(y, pr)[0]


def loo_spearman_ridge(X_accel, X_demo, y, K=20, alpha=20):
    """Clinic PerBout: Spearman Top-K + Demo, Ridge."""
...
def loo_spearman_vote(X_accel, X_demo, y, K=20, alpha=20):
    """Home model: Spearman Top-K + Demo, Vote(Ridge+Lasso+SVR)."""
    n_accel = X_accel.shape[1]
    pr_r = np.zeros(len(y))
    pr_l = np.zeros(len(y))
    pr_s = np.zeros(len(y))
    for i in range(len(y)):
        ...
        pr_r[i] = Ridge(alpha=alpha).fit(X_tr_s, y[tr]).predict(X_te_s)[0]
        pr_l[i] = Lasso(alpha=5, max_iter=5000).fit(X_tr_s, y[tr]).predict(X_te_s)[0]
        pr_s[i] = SVR(kernel='rbf', C=500, gamma=0.05).fit(X_tr_s, y[tr]).predict(X_te_s)[0]
    pr = (pr_r + pr_l + pr_s) / 3
    return r2_score(y, pr), mean_absolute_error(y * FT2M, pr * FT2M), spearmanr(y, pr)[0]
...
if __name__ == '__main__':
    ...
    print(f'\nComputing results (Clinic=Ridge, Home=Vote(Ridge+Lasso+SVR))...')
    rows = []

    # ── Row 1: Gait ──
    cr2, cmae, crho = loo_ridge(c_gait, y, alpha=5)
    hr2, hmae, hrho = loo_spearman_vote(h_gait, None, y, K=11, alpha=5)
    rows.append({'Feature Set': 'Gait', '#f': 'Clinic: 11, Home: 11',
    ...
    hr2, hmae, hrho = loo_spearman_vote(h_cwt, None, y, K=11, alpha=20)
    rows.append({'Feature Set': 'CWT', '#f': 'Clinic: 28, Home: 11',
                 'Clinic R²': round(cr2, 3), 'Clinic MAE (m)': round(cmae, 1), 'Clinic ρ': round(crho, 3),
                 'Home R²': round(hr2, 3), 'Home MAE (m)': round(hmae, 1), 'Home ρ': round(hrho, 3)})
    ...
    hr2, hmae, hrho = loo_spearman_vote(h_ws, None, y, K=11, alpha=5)
    rows.append({'Feature Set': 'WalkSway', '#f': 'Clinic: 12, Home: 11',
                 'Clinic R²': round(cr2, 3), 'Clinic MAE (m)': round(cmae, 1), 'Clinic ρ': round(crho, 3),
                 'Home R²': round(hr2, 3), 'Home MAE (m)': round(hmae, 1), 'Home ρ': round(hrho, 3)})
    ...
    hr2, hmae, hrho = loo_vote_no_selection(X_demo_bmi, y, alpha=20)
    rows.append({'Feature Set': 'Demo', '#f': 'Clinic: 4, Home: 4',
                 'Clinic R²': round(cr2, 3), 'Clinic MAE (m)': round(cmae, 1), 'Clinic ρ': round(crho, 3),
                 'Home R²': round(hr2, 3), 'Home MAE (m)': round(hmae, 1), 'Home ρ': round(hrho, 3)})
    ...
    hr2, hmae, hrho = loo_spearman_vote(h_pb, None, y, K=20, alpha=20)
    rows.append({'Feature Set': 'PerBout-Top20', '#f': 'Clinic: 20, Home: 20',
                 'Clinic R²': round(cr2, 3), 'Clinic MAE (m)': round(cmae, 1), 'Clinic ρ': round(crho, 3),
                 'Home R²': round(hr2, 3), 'Home MAE (m)': round(hmae, 1), 'Home ρ': round(hrho, 3)})
    ...
    hr2, hmae, hrho = loo_spearman_vote(h_pb, X_demo_bmi, y, K=20, alpha=20)
    rows.append({'Feature Set': 'PerBout-Top20+Demo', '#f': 'Clinic: 24, Home: 24',
                 'Clinic R²': round(cr2, 3), 'Clinic MAE (m)': round(cmae, 1), 'Clinic ρ': round(crho, 3),
                 'Home R²': round(hr2, 3), 'Home MAE (m)': round(hmae, 1), 'Home ρ': round(hrho, 3)})
    ...
    hr2, hmae, hrho = loo_spearman_vote(X_home_gcw, X_demo_bmi, y, K=20, alpha=20)
    rows.append({'Feature Set': 'Gait+CWT+WS+Demo', '#f': 'Clinic: 55, Home: 24',
                 'Clinic R²': round(cr2, 3), 'Clinic MAE (m)': round(cmae, 1), 'Clinic ρ': round(crho, 3),
                 'Home R²': round(hr2, 3), 'Home MAE (m)': round(hmae, 1), 'Home ρ': round(hrho, 3)})
    ...
    print(f'Home model:   Vote(Ridge(α)+Lasso(α=5)+SVR(C=500,γ=0.05))')
